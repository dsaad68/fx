...
/// OpenRouter over the host's streaming fetch port. The native provider opens
/// its own socket, which a WebAssembly host cannot do; this routes the same
/// request builder and the same SSE reduction through the embedder instead.
const provider_context = host_stream_provider.initContextWithProtocol(
    openrouter.buildRequest,
    .{ .resolve = openrouter.chatUrl },
    .{
        .context = null,
        .open_fn = open,
        .status_fn = status,
