...
/// The OpenRouter catalog over the host's blocking fetch port, parsed by the
/// same reader the native transport uses.
...
fn fetch(
    ...
    const url = openrouter_models.modelsEndpoint();

    const Header = struct { name: []const u8, value: []const u8 };
    var headers: std.ArrayList(Header) = .empty;
    defer headers.deinit(alloc);
    try headers.append(alloc, .{ .name = "accept", .value = "application/json" });
    // OpenRouter publishes the catalog unauthenticated, so a missing key still
    // yields the full list rather than a failure.
    ...
    const catalog = openrouter_models.parseCatalog(
        alloc,
